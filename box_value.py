...
df_advanced_input.drop(neo4j_cols, axis=1, inplace=True)
...
random_seed = 38
X_train_ad, X_val_ad, y_train_ad, y_val_ad = train_test_split(df_advanced_input, Y, test_size=0.2)
...
print('训练集准确率：\n', LR.score(X_train_ad, y_train_ad))  # 分数
print('验证集准确率：\n', LR.score(X_val_ad, y_val_ad))

y_advanced_pred_LR = (LR.predict(X_val_ad)).astype(int)

a = (y_val_ad.values).astype(int) / 1
b = y_advanced_pred_LR / 1
print(rmse(a, b))

# XGB
import xgboost as xgb

dtrain = xgb.DMatrix(X_train_ad, label=y_train_ad)
dtest = xgb.DMatrix(X_val_ad)
...
y_advanced_pred_xgb = (xb.predict(dtest)).astype(int)

a = (y_val_ad.values).astype(int) / 1
b = y_advanced_pred_xgb / 1
rmse(a, b)
...
model_lgb.fit(X_train_ad, y_train_ad)
y_advanced_pred_lgb = model_lgb.predict(X_val_ad)

a = (y_val_ad.values).astype(int) / 1
b = y_advanced_pred_lgb / 1
rmse(a, b)
...
gbm.fit(X_train_ad, y_train_ad)

print('训练集准确率：\n', gbm.score(X_train_ad, y_train_ad))  # 分数
print('验证集准确率：\n', gbm.score(X_val_ad, y_val_ad))

y_advanced_pred_gbm = (gbm.predict(X_val_ad)).astype(int)

a = (y_val_ad.values).astype(int)
b = y_advanced_pred_gbm
rmse(a, b)
